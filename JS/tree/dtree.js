...
// Get all selected nodes value (e.g. 1,2,3,4,5)
...
var rightTree = new dTree('rightTree');
//var hash = new Hashtable();
var hashCompany = new Hashtable();

function CreateTree(arrAreaRegion, arrRegionCountry, arrCountryCompany, divId, mode) {
    if (mode != "" && (mode == "General" || mode == "Special")) {
        treeMode = mode;
    }

    rightTree.add('Root', '-1', 'Statutory', 'false');
    ...
    for (var i = 0; i < arrAreaRegion.length - 1; i++) {
        if (arrAreaRegion[i] != "") {
            ...
            if (arrTmp.length > 1) {
                rightTree.add(arrTmp[0].trim() + "1", "Root", arrTmp[0], 'false');
                rightTree.add(arrTmp[1].trim() + "11", arrTmp[0].trim() + "1", arrTmp[1], 'false');
            }
        }
    }
    ...
    for (var j = 0; j < arrRegionCountry.length - 1; j++) {
        if (arrRegionCountry[j] != "") {
            ...
            if (arrTmp.length > 1) {
                rightTree.add(arrTmp[1].trim() + "111", arrTmp[0].trim() + "11", arrTmp[1], 'false');
            }
        }
    }
    ...
    for (var l = 0; l < arrCountryCompany.length - 1; l++) {
        if (arrCountryCompany[l] != "") {
            ...
            if (arrTmp.length > 1) {
                t = arrTmp[1].split(";#");
                if (t.length > 1) {
                    hashCompany.add(t[1], t[0]);
                    rightTree.add(t[1], arrTmp[0].trim() + "111", t[1], 'false');
                }
            }
        }
    }

    document.getElementById(divId).innerHTML = rightTree;
}

//Get all selected companies (e.g. 1;#isoftstone|2;#Microsoft)
function getSelectedCompany() {
    var selectedNodesTxt = rightTree.getText();
    var selectedCompanies = "";
    var chk;

    for (var i = 0; i < selectedNodesTxt.length; i++) {
        if (hashCompany.contains(selectedNodesTxt[i])) {
            selectedCompanies += hashCompany.items(selectedNodesTxt[i]) + ";#" + selectedNodesTxt[i] + "|";
            chk = document.getElementById("crightTree" + selectedNodesTxt[i]);
            if (chk) {
                chk.disabled = true;
            }
        }
...
//Remove the specified companies from the tree (parameter: e.g. 1;#isoftstone|2;#Microsoft)
function removeSelectedCompany(selectedCompanies) {
    ...
    for (var i = 0; i < arr.length; i++) {
        arrTmp = arr[i].split(";#");
        if (arrTmp.length > 1) {
            chk = document.getElementById("crightTree" + arrTmp[1]);
            if (chk) {
                chk.checked = chk.disabled = false;
                rightTree.checkNode(arrTmp[1]);
            }
        }
    }
}
